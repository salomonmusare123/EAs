...
CTrade trade;double previous_OP;int psell ; int pbuy ;int counter  = 0;int previous_pips= 0 ; int total ;double lowest ; double current_OP; input double multiplier = 1.6; input int minilot = 1; input int minilotb = 1; double lot = minilot ; double lotb = minilot;//1.04293 
...
void OnTick()
  ...
  if(currenttimestamp!=timestamplast){Print("-----------------------pips : " , pips);timestamplast = currenttimestamp ; previous_pips = pips; psell = len ; pbuy = lenb;
      ...
      if (profits>= plm ){closeAllSellPositions();lot = minilot;       trade.Sell(lot/100, NULL , NULL, NULL) ; Print("--------------closing all and selling"); };
      if (profitb>= plmb ){closeAllBuyPositions();lotb = minilotb; trade.Buy(lotb/100, NULL , NULL, NULL) ; Print("--------------closing all and buying");};
      ...
      if (pips<-30 && counter >1){lotb = MathRound( lotb *multiplier ); trade.Buy(lotb/100, NULL , NULL, NULL) ;lenb  = lenb+1 ; Print("_____________continue buy as pip = " , pips); }
      if (lowest>profitb){lowest = profitb;};if (lowest>profits){lowest = profits;}
      Comment( counter , "                                         previous_OP : " , previous_OP , " current_OP : " ,current_OP , " pips : " , MathRound(pips), " previous_pips : ",MathRound(previous_pips), "\n",
