...
double ctr = 0.042;double ctrb= 0.042;double len = 1; //code to get total number of open sell trades ,       ctr = contrade
double plm = 4.2 ;double lenb = 1;double plmb = 4.2 ;  //code to get total number of open buy trades
...
void OnTick()
  { plm = 4.2; plmb = 4.2 ;if (oneOrder == 1){debug = 1;trade.Buy(loot, NULL , NULL, NULL); Ask = NormalizeDouble(SymbolInfoDouble(_Symbol,SYMBOL_ASK), _Digits);trade.Sell(loot, NULL , NULL, NULL); Bid = NormalizeDouble(SymbolInfoDouble(_Symbol,SYMBOL_BID), _Digits);Comment("sending order");oneOrder += 2;};bool salomon = PositionSelect(_Symbol);
   ...
   if(len >0){plm = 4.2*len;};if(lenb >0){plmb = 4.2*lenb;};  tracker = tracker + 1;aaction = 0;aactionb = 0;
...
void step(int saaction , int slen){
   if (saaction == 1){
      ...
      trade.Sell(loot, NULL , NULL, NULL); // (Ask - 1000*_Point) this is stop loss;
      }
   if (saaction == 2){ 
      double lotie = loot*(MathPow(2,slen + 1));
...
void stepb(int saactionb , int slenb){
   if (saactionb == 1){ 
      ...
      trade.Buy(loot, NULL , NULL, NULL); // (Ask - 1000*_Point) this is stop loss;
      }
   if (saactionb == 2){ 
      double lotieb = loot*(MathPow(2,slenb + 1));
