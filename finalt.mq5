...
double ctr = 0.0042;double ctrb= 0.0042;double len = 1; //code to get total number of open sell trades ,       ctr = contrade
double plm = 0.42 ;double lenb = 1;double plmb = 0.42 ;  //code to get total number of open buy trades
...
void OnTick()
  { plm = 0.42; plmb = 0.42 ;if (oneOrder == 1){debug = 1;trade.Buy(loot, NULL , NULL, NULL); Ask = NormalizeDouble(SymbolInfoDouble(_Symbol,SYMBOL_ASK), _Digits);trade.Sell(loot, NULL , NULL, NULL); Bid = NormalizeDouble(SymbolInfoDouble(_Symbol,SYMBOL_BID), _Digits);Comment("sending order");oneOrder += 2;};bool salomon = PositionSelect(_Symbol);
   ...
   if(len >0){plm = 0.42*len;};if(lenb >0){plmb = 0.42*lenb;};  tracker = tracker + 1;aaction = 0;aactionb = 0;
...
void stepb(int saactionb , int slenb){
   ...
   if (saactionb == 2){ 
      double lotieb = loot*(2*slenb + 1)*2;
